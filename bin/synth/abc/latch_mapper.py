...
class Latch(object):
    ...
    def __init__(self, instance_id, d, q, ck, gtype):
        self.instance_id = instance_id
        self.d, self.q, self.ck = d, q, ck
        self.gtype = gtype

        # FIXME
        self.dpin, self.qpin, self.ckpin = (None,)*3

    def print_latch(self, digit):
        return "%s l%0*d ( .%s(%s), .%s(%s), .%s(%s) );" % \
                (self.gtype, digit, self.instance_id,
                 self.dpin, self.d, self.qpin, self.q, self.ckpin, self.ck)
...
class LatchMapper(object):
    ...
    def __init__(self, clock_port, latch_cell):
        ...
        self.latch_dpin, self.latch_qpin, self.latch_ckpin = None, None, None
        self.tie_hi, self.tie_lo = None, None
            ...
            if tokens[0] == '//':
                ...
                while True:
                    ...
                    latch = Latch(instance_id, d, q, self.clock_port, 
                                  self.latch_cell)
                    latch.dpin = self.latch_dpin
                    latch.qpin = self.latch_qpin
                    latch.ckpin = self.latch_ckpin
    ...
    def map_latches(self, filename):
        ...
        # FIXME: Raise error if latch_cell and related variables are None.
